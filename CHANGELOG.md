...
## [0.3.10] - 2025-11-24
### Added
- JavaScript syntax highlighting for `script` blocks (without `<script>` tags). (#7)
- Python syntax highlighting for `on change` blocks. (#6)

### Fixed
- `.using()` pattern now closes properly when commented out. (#5)
- Resolved VS Code conflict warning: "Overwriting grammar scope name to file mapping for scope source.yaml".
